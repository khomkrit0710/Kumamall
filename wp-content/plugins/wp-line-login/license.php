...
class License
{
    public function __construct()
    {
         ...
         */
        $this->product_unique_id = 'BP-LINE-LOGIN';
        $this->sl_product_id = $this->product_unique_id;


        /**
         * This plugin_name property used for display to user
         */
        $this->plugin_name = 'WP LINE Login';

        
        /**
         * Plugin file name 
         */
        $this->plugin_filename = 'wp-line-login.php';
        ...
        /**
         * License host url
         * Create 2 constants in wp-config.php for local environment settings
         * If 2 constants are not set, https://boostpress.com/index.php is default
         * LICENSE_ENVIRONMENT (local)
         * LICENSE_HOST_URI local site url
         */
        $this->sl_app_api_url = 'https://boostpress.com/index.php';
        if (defined('LICENSE_ENVIRONMENT') && LICENSE_ENVIRONMENT == 'local') {
            $this->sl_app_api_url = LICENSE_HOST_URI;
        }
         ...
         */
        $this->language_domain = 'wp-line-login';


        /**
         * Slug is folder's name
         */
        $this->slug = 'wp-line-login';
            ...
            if (is_wp_error($data) || $data['response']['code'] != 200) {
                ...
                if (isset($data_body->status)) {
                    if ($data_body->status == 'success' && $data_body->status_code == 's100') {
                        ...
                        add_action('admin_notices', function () use ($data_body) {
                            ?>
                            <div class="notice notice-error">
                                <p>
                                    <?php echo sprintf(
                                        '%s. ( <strong>Error Code:</strong> %s, <strong>Error Message:</strong> %s.)',
                                        __('There was a problem activating the license', $this->language_domain),
                    ...
                    add_action('admin_notices', function () use ($data_body) {
                        ...
                        <div class="notice notice-error">
                            ...
                            <p>
                                <?php echo sprintf(
                                        '%s. ( <strong>Error Code:</strong> %s, <strong>Error Message:</strong> %s. )',
                                        __('There was a problem establishing a connection to the API server. Please try again', $this->language_domain),
                                        $data_body->status_code,
                                        $data_body->message); 
                                ?>
                                </p>
                    ...
                    if ($data_body->status == 'success' && $data_body->status_code == 's201') {
                        ...
                        add_action('admin_notices', function () use ($data_body){
                            ...
                            <div class="notice notice-error">
                                ...
                                <?php echo sprintf(
                                        '%s. ( <strong>Error Code:</strong> %s, <strong>Error Message:</strong> %s. )',
                                        __('There was a problem deactivating the license', $this->language_domain),
                                        $data_body->status_code,
                                        $data_body->message); 
                                ?>
                                </p>
    ...
    public function check_for_plugin_update($checked_data)
    {
        ...
        if  ( $data    === FALSE )
        {
            // If license server is on local
            // Unsafe remote get data should allow
            if (defined('LICENSE_ENVIRONMENT') && LICENSE_ENVIRONMENT == 'local') {
