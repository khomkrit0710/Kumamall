    ...
    class WC_Gateway_2c2p extends WC_Payment_Gateway {
        ...
        function generate_2c2p_form($order_id) {
            ...
            if (!$isValid) {                
                ...
                echo '</br>';
                echo '&nbsp;&nbsp;&nbsp;&nbsp<a class="button cancel" href="' . esc_url($order->get_cancel_order_url()) . '">' . esc_html__('ยกเลิกคำสั่งซื้อ','woo_2c2p') . '</a>';
                return;
            }
            else{

                echo '<p><strong>' . esc_html__('ยืนยันคำสั่งซื้อ','woo_2c2p') . '</strong> <br/>' . esc_html__('ยืนยันคำสั่งซื้อ เพื่อทำการชำระเงิน!','woo_2c2p') . '</p>';
            }
            ...
            $strHtml .= '<input type="submit" class="button-alt" id="submit_2c2p_payment_form" value="' . esc_html__('ชำระเงิน','woo_2c2p') . '" />';
            $strHtml .= '&nbsp;&nbsp;&nbsp;<a class="button cancel" href="' . esc_url($order->get_cancel_order_url()) . '">' . esc_html__('ยกเลิกคำสั่งซื้อ','woo_2c2p') . '</a>';
            $strHtml .= '</form>';
                        ...
                        if ($order->status !== 'completed') {
                            ...
                            if ($isValidHash) {
                                ...
                                if (strcasecmp($payment_status, "000") == 0) { //Success using credit/debit card (Authorized) or Success when paid with cash channel (Paid)
                                    ...
                                    $this->msg['message'] = 'คุณชำระเงินสำเร็จ และสามารถตรวจสอบการจัดส่งได้ใน "คำสั่งซื้อของคุณ"' ;
                                    ...
                                    $trans_authorised     = true;
                                    $this->msg['message'] = "ขอบคุณสำหรับคำสั่งซื้อ สถานะการชำระเงินจะแจ้งให้ทราบผ่านทาง Email";
                                    ...
                                    $this->msg['message'] = "ธุรกรรมไม่สำเร็จ";
                                ...
                                $this->msg['message'] = "เกิดข้อผิดพลาดด้านความปลอดภัย พบการเข้าถึงระบบโดยไม่ได้รับอนุญาต";
